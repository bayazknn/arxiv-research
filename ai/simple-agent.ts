...
const getPdfText = tool(
  ...
  // }
    ...
    try {
      ...
      const pdfContent = `the pdf context text is:\n\n\n\n ${text}`
      return pdfContent
    } catch (error: any) {
      console.error("Error in getPdfText tool:", error);
      return "I am sorry, I cannot fulfill this request. The getPdfText tool is not working correctly.";
    }
    return ""
...
const prompt = (
    ...
    const systemMsg = `"You are a scientific advisor to analyze arxiv papers. You MUST use the getPdfText tool to extract text from the article. The pdf url is ${pdfUrl}. To use the getPdfText tool, you MUST call it with the following format: \\"getPdfText: {pdfUrl: \\"${pdfUrl}\\"}\\". The getPdfText tool will then return the extracted text from the PDF. Use this extracted text to answer the user's questions."`;
